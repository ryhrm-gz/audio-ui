...
import type { RangeSliderValueProps } from "./types.ts";

export const Value = forwardRef<HTMLSpanElement, RangeSliderValueProps>(function Value(props, ref) {
  const { index, render, children, format, ...elementProps } = props;
  const context = useRangeSliderContext("RangeSlider.Value");
  ...
  const content =
    typeof children === "function"
      ...
      : (children ??
        ...
        (index === undefined ? `${context.state.value[0]} - ${context.state.value[1]}` : value));
